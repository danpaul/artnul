class Line {
    ...
    jagged({ start, end, segments = 10, draw = null, random = 0 }, context, sketch) {
      const points = this.getLineSegments({ start, end, segments });
      ...
      for (let i = 1; i < points.length; i++){
        ...
        if(draw){
            draw({ start: previousPoint, end: thisPoint }, context);
        } else {
