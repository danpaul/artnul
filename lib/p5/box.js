...
const centeredBox = ({ upperLeftPoint, lowerRightPoint, width, height }) => {
...
const colorBox = ({ upperLeftPoint, lowerRightPoint, colors, reduce = 0.5, even = true }) => {
    const { xDistance, yDistance } = pointDistance({ upperLeftPoint, lowerRightPoint });
    ...
    for(let i = 0; i < colors.length; i++){
        ...
        const box = centeredBox({ upperLeftPoint, lowerRightPoint, ...dimensions });
        fill(colors[i]);
        stroke(colors[i]);
        rect(box.upperLeftPoint.x, box.upperLeftPoint.y, box.width, box.height);
