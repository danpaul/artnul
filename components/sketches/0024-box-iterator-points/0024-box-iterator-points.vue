...
let OPACITY = 3
let ITERATIONS = 10
// const connectPoints = (sketch) => {
//   for(let i = 0; i < ITERATIONS; i++){
//     const point1 = points[Math.floor(sketch.random(points.length))];
//     const point2 = points[Math.floor(sketch.random(points.length))];
...
const connectPoints = (sketch) => {
  points.forEach((point) => {
    for(let i = 0; i < ITERATIONS; i++){
      // const point1 = points[Math.floor(sketch.random(points.length))];
      const point2 = points[Math.floor(sketch.random(points.length))];
      sketch.line(point[0], point[1], point2[0], point2[1])
    }
  })
  // for(let i = 0; i < ITERATIONS; i++){
  ...
  gridIterate({
    ...
    callback({ translate, upperLeftPoint, lowerRightPoint }) {
      if (Math.random() < RECURSE_PROBABILITY && iterationLevel < ITERATION_LIMIT) {
        ...
        points.push([translate.x(0.5), translate.y(0.5)])
        // console.log(translate.x(0.5))
        // const colors = [];
...
export default {
  ...
  methods: {
    setup(sketch) {
      ...
      sketch.background(0)
      sketch.stroke(255, 255, 255, OPACITY)
      // sketch.stroke(255)
      // asdf
      sketch.fill(0)
      sketch.noLoop()
